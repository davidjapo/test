...
public class Utilidades {
	...
	/*
	 * public static Casos construirCaso(String cadena) { String[] array =
	 * cadena.split(SEPARADOR); try { return new Casos(array[0],
	 * Date.parse(array[1]),Integer.parseInt(array[2])); } catch(ParseException pe)
	 * { pe.getMessage(); pe.printStackTrace(); return null; } }
	 */

	/*
	 * public static String construirCadena(Pedido pedido, String SEPARADOR) {
	 * return pedido.getProducto()+SEPARADOR+pedido.getUnidades()+SEPARADOR+pedido.
	 * getPrecioUnitario()+SEPARADOR+pedido.getSeccion()+SEPARADOR+sdf.format(pedido
	 * .getFecha()); }
	 */
	...
	// Utilidad para convetir de .json a Array y de Array a Stream:
	public static Stream<Caso> jsonToStream(){
		Gson gson = new Gson();	
		Reader lectura = null;
		try {
		...
		Caso[] casos = gson.fromJson(lectura,Caso[].class);
